...
class CTransformable {
 ...
 public:
    ...
    virtual void getParams(CMatrix &params) const {
        if (params.getRows() != 1) {
            throw ndlexceptions::RuntimeError(
                "getParams(): Dimension match check failed, numbers of rows should be 1, currently it is "
                + ndlstrutil::itoa(params.getRows()));
        }
        if (params.getCols() != getNumParams()) {
            throw ndlexceptions::RuntimeError("getParams(): Dimension match check failed, numbers of columns should be "
                                              + ndlstrutil::itoa(getNumParams()) + ", currently it is "
                                              + ndlstrutil::itoa(params.getRows()));
        }
    ...
    virtual void setParams(const CMatrix &params) {
        if (params.getRows() != 1) {
            throw ndlexceptions::RuntimeError(
                "setParams(): Dimension match check failed, numbers of rows should be 1, currently it is "
                + ndlstrutil::itoa(params.getRows()));
        }
        if (params.getCols() != getNumParams()) {
            throw ndlexceptions::RuntimeError("setParams(): Dimension match check failed, numbers of columns should be "
                                              + ndlstrutil::itoa(getNumParams()) + ", currently it is "
                                              + ndlstrutil::itoa(params.getRows()));
        }
        for (unsigned int i = 0; i < params.getCols(); i++)
    ...
    virtual void getTransParams(CMatrix &transParam) const {
        if (transParam.getRows() != 1) {
            throw ndlexceptions::RuntimeError(
                "getTransParams(): Dimension match check failed, numbers of rows should be 1, currently it is "
                + ndlstrutil::itoa(transParam.getRows()));
        }
        if (transParam.getCols() != getNumParams()) {
            throw ndlexceptions::RuntimeError(
                "getTransParams(): Dimension match check failed, numbers of columns should be "
                + ndlstrutil::itoa(getNumParams()) + ", currently it is " + ndlstrutil::itoa(transParam.getRows()));
        }
    ...
    virtual void setTransParams(const CMatrix &transParam) {
        if (transParam.getRows() != 1) {
            throw ndlexceptions::RuntimeError(
                "setTransParams(): Dimension match check failed, numbers of rows should be 1, currently it is "
                + ndlstrutil::itoa(transParam.getRows()));
        }
        if (transParam.getCols() != getNumParams()) {
            throw ndlexceptions::RuntimeError(
                "setTransParams(): Dimension match check failed, numbers of columns should be "
                + ndlstrutil::itoa(getNumParams()) + ", currently it is " + ndlstrutil::itoa(transParam.getRows()));
        }
    ...
    virtual void getGradTransParams(CMatrix &g) const {
        if (g.getRows() != 1) {
            throw ndlexceptions::RuntimeError(
                "getGradTransParams(): Dimension match check failed, numbers of rows should be 1, currently it is "
                + ndlstrutil::itoa(g.getRows()));
        }
        if (g.getCols() != getNumParams()) {
            throw ndlexceptions::RuntimeError(
                "getGradTransParams(): Dimension match check failed, numbers of columns should be "
                + ndlstrutil::itoa(getNumParams()) + ", currently it is " + ndlstrutil::itoa(g.getRows()));
        }
        getGradParams(g);
        double      val;
        double      param;
