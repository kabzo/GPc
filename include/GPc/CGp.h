...
class CGp : public CMapModel, public CProbabilisticOptimisable, public CStreamInterface, public CMatInterface {
 ...
 private:
    ...
    void _posteriorMean(CMatrix &mu,
                        const CMatrix &kX) const; /// compute the posterior mean given the kernel evaluated at test points.
    void _posteriorVar(CMatrix &varSigma,
                       CMatrix &kX,
                       const CMatrix &Xin) const; /// compute the posterior variance given the kernel evaluated at test points.
    double          jitter;
