...
class CKern : public CMatInterface, public CStreamInterface, public CTransformable, public CRegularisable {
 public:
    ...
    virtual void getGradX(CMatrix &g,
                          const CMatrix &X,
                          unsigned int pointNo,
                          const CMatrix &X2,
                          bool addG = false) const =0;
    ...
    virtual void getGradParams(CMatrix &g,
                               const CMatrix &X,
                               const CMatrix &X2,
                               const CMatrix &cvGrd,
    ...
    virtual void getGradParams(CMatrix &g, const CMatrix &X, const CMatrix &cvGrd, bool regularise = true) const {
        ...
        if (regularise) {
            addPriorGrad(g);
        } /// don't forget to add prior gradient at the end.
    }
    ...
    void getGradTransParams(CMatrix &g,
                            const CMatrix &X,
                            const CMatrix &X2,
                            const CMatrix &cvGrd,
                            bool regularise = true) const;
...
class CComponentKern : public CKern {
 public:
    ...
    virtual void addPrior(CDist *prior, unsigned int index) {
        throw ndlexceptions::Error(
            "Error cannot add priors to component kernels directly, please add to the components.");
    }
