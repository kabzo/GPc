...
void CNoise::fromMxArray(const mxArray *matlabArray) {
    ...
    if (mxType != getType()) {
        throw ndlexceptions::FileReadError(
            "Error mismatch between saved type, " + mxType + ", and Class type, " + type + ".");
    }
...
//     throw ndlexceptions::StreamFormatError("type", "Error mismatch between saved type, " + ttype + ", and Class type, " + getType() + ".");
    ...
    if (numPar == getNumParams()) {
        ...
        throw ndlexceptions::StreamFormatError("numParams",
                                               "Number of parameters in file does not match computed number.");
    }
}
...
void CScaleNoise::out(CMatrix &yPred, const CMatrix &muTest, const CMatrix &varSigmaTest, const bool variance) const {
...
void CScaleNoise::out(CMatrix &yPred,
                      CMatrix &errorBars,
...
void CProbitNoise::out(CMatrix &yOut,
                       CMatrix &probOut,
                       const CMatrix &muTest,
                       const CMatrix &varSigmaTest,
                       const bool variance) const {
...
void CProbitNoise::out(CMatrix &yOut, const CMatrix &muTest, const CMatrix &varSigmaTest, const bool variance) const {
...
void CNcnmNoise::test(const CMatrix &muout, const CMatrix &varSigmaOut, const CMatrix &yTest) const {
    ...
    for (unsigned int j = 0; j < getOutputDim(); j++) {
        ...
        miss = miss * 100.0;
        cout << "Classification error on output " << j + 1 << ": " << err << "%, with " << miss << " missing values."
             << endl;
    }
}

void CNcnmNoise::out(CMatrix &yOut, const CMatrix &muTest, const CMatrix &varSigmaTest, const bool variance) const {
    DIMENSIONMATCH(yOut.dimensionsMatch(muTest));
    DIMENSIONMATCH(muTest.dimensionsMatch(varSigmaTest));
    for (unsigned int j = 0; j < yOut.getCols(); j++) {
        double            b = bias.getVal(j);
...
void CNcnmNoise::out(CMatrix &yOut,
                     CMatrix &probOut,
                     const CMatrix &muTest,
                     const CMatrix &varSigmaTest,
                     const bool variance) const {
...
void COrderedNoise::getGradParams(CMatrix &g) const {
    ...
    for (unsigned int j = 0; j < getOutputDim(); j++) {
        ...
        for (unsigned int i     = 0; i < getNumData(); i++) {
            ...
            } else if (isnan(getTarget(i, j))) { /* do nothing*/  }
            else {
...
void COrderedNoise::out(CMatrix &yOut, const CMatrix &muTest, const CMatrix &varSigmaTest, const bool variance) const {
    DIMENSIONMATCH(yOut.dimensionsMatch(muTest));
    DIMENSIONMATCH(muTest.dimensionsMatch(varSigmaTest));
    for (unsigned int j = 0; j < yOut.getCols(); j++) {
        double            b = bias.getVal(j);
...
void COrderedNoise::out(CMatrix &yOut,
                        CMatrix &probOut,
                        const CMatrix &muTest,
                        const CMatrix &varSigmaTest,
                        const bool variance) const {
        ...
        for (unsigned int i = 0; i < muTest.getRows(); i++) {
            ...
            if (targVal == 0) {
                ...
                for (unsigned int k = 0; k < targVal - 1; k++)
                    muAdj += widths.getVal(k);
                L.setVal(
                    ndlutil::cumGaussian(muAdj * c) - ndlutil::cumGaussian((muAdj - widths.getVal(targVal - 1)) * c), i,
                    j);
            } else if (targVal == getNumCategories() - 1) {
                for (unsigned int k = 0; k < targVal - 1; k++)
                    muAdj += widths.getVal(k);
...
CNoise *readNoiseFromStream(istream &in) {
    ...
    if (tbaseType != "noise") {
        throw ndlexceptions::StreamFormatError("baseType", "Error mismatch between saved base type, " + tbaseType
                                                           + ", and Class base type, noise.");
    }
    CNoise *pnoise;
