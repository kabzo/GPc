        ...
        switch (getApproximationType()) {
            ...
            case DTCVAR:
                if (isSpherical()) {
                    ...
                    throw ndlexceptions::NotImplementedError(
                        "Non-spherical implementations not yet in place for DTC or DTCVAR");
                }
...
double CGp::logLikelihood() const {
    ...
    switch (getApproximationType()) {
        ...
        case FITC:
            if (isSpherical()) {
                L += ((double) numActive - (double) getNumData()) * log(getBetaVal())
                     + (double) getNumData() * ndlutil::LOGTWOPI;
                for (unsigned int i    = 0; i < getNumData(); i++) {
...
void CGp::updateG() const {
    ...
    switch (getApproximationType()) {
        ...
        case PITC:
            if (isOptimiseX()) {
                throw ndlexceptions::NotImplementedError(
                    "Optimisation of X not yet implemnted for PTIC sparse approximations");
            }
    ...
    if (isSparseApproximation()) {
        if (isInducingFixed()) {
            const char *fieldNames[] =
                           {"d", "q", "k", "N", "learnScales", "approx", "fixInducing", "inducingIndices", "scale",
                            "bias", "beta", "betaTransform", "type"};
            ...
            const char *fieldNames[] =
                           {"d", "q", "k", "N", "learnScales", "approx", "fixInducing", "X_u", "scale", "bias", "beta",
                            "betaTransform", "type"};
            matlabArray = mxCreateStructArray(1, dims, 13, fieldNames);
...
void CGp::fromMxArray(const mxArray *matlabArray) {
    ...
    if (mxType != getType()) {
        throw ndlexceptions::FileReadError(
            "Error mismatch between saved type, " + mxType + ", and Class type, " + getType() + ".");
    }
    ...
    if (isSparseApproximation()) {
        ...
        if (betaTransformType == "exp") {
            ...
            ndlexceptions::NotImplementedError(
                "fromMxArray: Not yet implemented transform types apart from exp and negLogLogit.");
        }
    }
...
void CGp::readParamsFromStream(istream &in) {
    ...
    if (tbaseType != getBaseType()) {
        throw ndlexceptions::StreamFormatError("baseType", "Error mismatch between saved base type, " + tbaseType
                                                           + ", and Class base type, " + getBaseType() + ".");
    }
    string ttype = getTypeStream(in);
    if (ttype != getType()) {
        throw ndlexceptions::StreamFormatError("type",
                                               "Error mismatch between saved type, " + ttype + ", and Class type, "
                                               + getType() + ".");
    }
